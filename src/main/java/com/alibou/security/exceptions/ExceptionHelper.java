...
import com.alibou.security.payload.ErrorData;
...
public class ExceptionHelper {
    ...
    public ResponseEntity<?> handleException(ChangeSetPersister.NotFoundException ex) {
        ex.printStackTrace();
        return new ResponseEntity<>(ApiResult.errorResponse(MessageConstants.NOT_FOUND, HttpStatus.NOT_FOUND.value()), HttpStatus.BAD_REQUEST);
    }
    ...
    public ResponseEntity<ApiResult<ErrorData>> handleException(EmptyResultDataAccessException ex) {
        ex.printStackTrace();
        //AGAR RESOURSE TOPILMAGANLIGI XATOSI BO'LSA CLIENTGA QAYSI TABLEDA NIMA TOPILMAGANLIGI HAQIDA XABAR QAYTADI
        return new ResponseEntity<>(ApiResult.errorResponse(ex.getMessage(), HttpStatus.BAD_REQUEST.value()), HttpStatus.BAD_REQUEST);
    }
